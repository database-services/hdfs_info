...
def parse_quota_info(quota_output):
    """Parse quota information into a JSON format."""
    quota_info = []
    for line in quota_output.splitlines():
        ...
        if len(parts) >= 6:
            ...
            quota_used = parts[1]
            space_quota = parts[2]
            space_quota_used = parts[3]
            path = ' '.join(parts[5:])
            entry = {
                "quota": quota,
                "quota_used": quota_used,
                "space_quota": space_quota,
                "space_quota_used": space_quota_used,
                "path": path
            }
            quota_info.append(entry)
    return quota_info


def get_snapshot_details(snapshot_dir):
    """Get details of all snapshots in a given directory."""
    snapshot_details = []
    snapshots_output = run_hdfs_command(['hdfs', 'dfs', '-ls', snapshot_dir])
    for line in snapshots_output.splitlines():
        if line.startswith('Found'):
            continue  # Ignore the "Found X items" line
        parts = line.split()
        if len(parts) >= 8:
            permissions = parts[0]
            replication = parts[1]
...
    hdfs_info['configuration'] = get_all_hdfs_configurations()
...
    hdfs_info['dfsadmin'] = interpret_dfsadmin_report(hdfs_runtime)
    ...
    # Get HDFS safe mode status
    hdfs_info['safemode'] = run_hdfs_command([hdfs_cmd, 'dfsadmin', '-safemode', 'get'])

    # Get quota information
    quota_info = run_hdfs_command([hdfs_cmd, 'dfs', '-count', '-q', '/'])
    hdfs_info['quota'] = parse_quota_info(quota_info)

    # Get HDFS snapshot information
    snapshot_dirs_output = run_hdfs_command([hdfs_cmd, 'lsSnapshottableDir'])
    snapshot_dirs = snapshot_dirs_output.splitlines()
    snapshot_info = {}
    for snapshot_dir in snapshot_dirs:
        snapshot_info[snapshot_dir] = get_snapshot_details(snapshot_dir)
    hdfs_info['snapshots'] = snapshot_info
...
    write_to_json(hdfs_info, 'hdfs_info.json')
